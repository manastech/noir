...
pub fn run(
    ...
    rt.block_on(async {
        local.run_until(async move {
            ...
            let manager = spawn_local(async move {
                        ...
                        "foo" => {
                            ...
                            match result {
                                DebugCommandResult::Ok => {
                                    let loc = context.get_current_opcode_location();
                                    println!("GOT OK!");
                                    match loc {
                                        Some(loc) => { println!("Loc: {}", loc); },
                                        None => { println!("Loc unkwnown"); }
                                    }
                                },
                                DebugCommandResult::Done => { print!("GOT DONE!"); },
                                _ => { println!("BOOM (I guess?)"); }
                            }
                        }
