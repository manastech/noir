...
fn build_source_to_opcode_debug_mappings(
    ...
    result
        .iter_mut()
        .for_each(|(_, file_locations)| file_locations.sort_by_key(|x| (x.0, x.1)));

    // debugging output
    result.iter().for_each(|(file, locations)| {
        eprintln!("File: {file:?}");
        locations.iter().for_each(|(line, opcode)| {
            eprintln!("  > {line} {opcode}");
        });
        eprintln!("");
    });
